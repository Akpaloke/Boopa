...
This project is a static React document, so deployment configuration is injected through `config.js`.
...
Never place `PAYSTACK_SECRET_KEY` or `SUPABASE_SERVICE_ROLE_KEY` in `config.js`, HTML, frontend environment variables, or browser code.
