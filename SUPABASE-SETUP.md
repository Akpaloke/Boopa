...
Apply all migrations in `supabase/migrations` before deploying, including `202609181430_profiles_consistency.sql`, `202609181500_signup_profile_trigger.sql`, and `202609182300_requested_fixes.sql`. The requested-fixes migration creates/configures the `avatars` bucket and its ownership policies and adds `communities.whatsapp_link`. The frontend reads profiles, communities, and posts from Supabase.
