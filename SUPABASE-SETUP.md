...
Apply all migrations in `supabase/migrations` before deploying, including `202609181430_profiles_consistency.sql`, `202609181500_signup_profile_trigger.sql`, `202609182300_requested_fixes.sql`, `202609191330_auth_rls.sql`, `202609191400_notifications_actor.sql`, and `202609191410_verification_reference.sql`. The requested-fixes migration creates/configures the `avatars` bucket and its ownership policies and adds `communities.whatsapp_link`; the auth migration scopes profile insert/update/read policies to the authenticated user id; the notifications migration restores the optional actor relationship for existing databases; the verification migration stores Paystack transaction references.
