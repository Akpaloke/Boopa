...
Deno.serve(async (req) => {
  ...
  if (event.event === "charge.success" && (data.amount !== 50000 || data.currency !== "NGN" || planCode !== PLAN_CODE)) return new Response("ok");
  ...
  if (status) await admin.from("verification_subscriptions").update({ status, paystack_customer_code: data.customer?.customer_code, paystack_subscription_code: data.subscription?.subscription_code || data.subscription_code, paystack_email_token: data.subscription?.email_token || data.email_token, paystack_plan_code: data.plan?.plan_code || PLAN_CODE, next_payment_date: data.next_payment_date, updated_at: new Date().toISOString() }).eq("user_id", user.id);
